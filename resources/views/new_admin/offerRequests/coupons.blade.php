                ...
                <tbody>
                    @if($coupons->count() > 0)
                        @foreach($coupons as $coupon)
                            <tr>
                                <td width="2%"><input id="coupon{{ $coupon->id }}"  class="form-check-input" type="checkbox" {{ $request->user_id != null &&$coupon->user_id==$request->user_id ? 'checked':'' }}  name='coupons[]' value="{{ $coupon->id }}"></td>
