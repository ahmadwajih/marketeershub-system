...
@section('content')
    ...
    <!--begin::Form-->
    <form id="kt_ecommerce_add_product_form" class="form d-flex flex-column flex-lg-row" action="{{ route('admin.publishers.update', $publisher->id) }}" method="POST" enctype="multipart/form-data">
        @method('PUT')
        @csrf
        <!--begin::Main column-->
                                        ...
                                        <div class="mb-10 fv-row">
                                            ...
                                            <select id="cities" name="city_id" data-control="select2" class="form-select form-select-sm">
                                                ...
                                                @foreach ($cities as $city)
                                                    <option {{ old('city_id') == $city->id ? "selected" : ($publisher->city_id == $city->id ? "selected" : '') }} value="{{ $city->id }}">{{  $city->name }}</option>
                                                @endforeach
                                            </select>
                                            <!--end::Input-->
