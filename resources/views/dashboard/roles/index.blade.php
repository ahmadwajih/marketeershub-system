...
@push('scripts')
<script>
    var route = "{{ getUrlAfterDomain(route('admin.roles.index')) }}";
</script>
    @if(Request::segment(2)=='trashed')
        <script src="{{asset('js/datatables/trashed/roles.js')}}"></script>
    @else
    <script src="{{asset('js/datatables/roles.js')}}"></script>
    @endif
